...
pub fn record_mise_projection(
    ...
    if projection.schema != crate::MISE_PLANNER_PROJECTION_SCHEMA {
        bail!(
            "Mise planner projection id {} is readable but not recordable; regenerate it with `papertiger-mise projection export`, then run `papertiger mise record <task> <projection>`",
            projection.schema
        );
    }
